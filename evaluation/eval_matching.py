...
前置条件:
    1. 主后端(8000)已启动
    2. 已跑过 eval_parse.py(简历已上传且解析成功)

指标定义(答辩口径):
    - M1 匹配命中率(核心指标): Top10 推荐岗位中, 岗位技能与简历技能交集 ≥ 阈值(技能数≥4 时取 2,
      否则取 1)的岗位占比, 再对所有简历取平均 → 即"推荐的岗位确实匹配"的比例
    - M2 平均技能重合度: 每个 Top10 岗位的 |岗位技能∩简历技能| / |简历技能| 的平均
    - M3 Top1 相关率: 排名第 1 的岗位, 其标题或技能与简历相关的简历占比
"""
import argparse
import json
