...
# __file__ 是 config.py 自己, 向上 3 层: config.py -> core -> app -> JobHunter
...
class Settings(BaseSettings):
    ...
    SEARCH_ENGINE: str = Field(default="es", description="职位搜索引擎: es或mysql")
    ...
    LLM_SERVICE_URL: str = Field(
        ...
        description="LLM 简历解析服务地址",
    )
    LLM_PARSE_TIMEOUT: int = Field(
        default=120,
        description="LLM 解析超时(秒)",
    )
    ...
    ZHIPU_BASE_URL: str = Field(
        ...
        description="智谱 API 基址",
    )
    ZHIPU_CHAT_MODEL: str = Field(default="glm-4-flash", description="对话模型")
    ...
    def CHROMA_PATH(self) -> Path:
        """ChromaDB 持久化目录(跟着 BASE_DIR 走, 项目移动自动适配)。"""
